...
import { Plus, Trash2, Save, ArrowLeft, BedSingle, BedDouble, Weight, BrickWall, Upload, Package, Palette, Ruler, Archive, Crown, Layers, Sparkles, Images, X, Star, Pencil, ArrowUp } from 'lucide-react';
import { Link, useParams } from 'react-router-dom';
import { ProductSize, ProductColor, ProductCategory, ProductSubcategory } from '@/types';
import { toast } from 'sonner';
import { updateProduct, uploadImage, getProductByName } from '@/api/api';
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
const initialColor: ProductColor[] = [{ name: 'Seal Grey Naples', hex: '#9f938c' }, { name: 'Cream Naples', hex: '#ded1c1' }, { name: 'Black Naples', hex: '#000000' }, { name: 'Slate Grey Naples', hex: '#55504d' }, { name: 'Mauve Naples', hex: '#8f7774' }, { name: 'Mink Naples', hex: '#825d43' }, { name: 'Cream Chenille', hex: '#d0c5b4' }, { name: 'Silver Chenille', hex: '#757575' }, { name: 'Charcoal Chenille', hex: '#515655' }, { name: 'Black Chenille', hex: '#000000' }, { name: 'Black Plush', hex: '#000000' }, { name: 'Steel Plush', hex: '#454545' }, { name: 'Cream Plush', hex: '#c1b5a4' }, { name: 'Silver Plush', hex: '#a09f9f' }, { name: 'Blue Plush', hex: '#133a54' }, { name: 'Latte Plush', hex: '#302118' }, { name: 'Emerald Plush', hex: '#2a4430' }, { name: 'Mink Plush', hex: '#9f856f' }, { name: 'Pink Plush', hex: '#bf9089' }];
...
export default function AdminProductEdit() {
    ...
    // Use id as storage folder or fallback
    const storageId = id || `product-${Date.now()}`;
            ...
            if (id) {
                try {
                    ...
                    setFormData({
                        ...
                        sizes: product.sizes || [],
                        colors: product.colors || initialColor,
                        storageOptions: product.storageOptions || [],
                        headboardOptions: product.headboardOptions || [],

                        baseOptions: product.baseOptions || [],
    ...
    const postProductData = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Recalculate discount price to be safe
            ...
            await updateProduct(finalProductData);
    ...
    const calculateFinalPrice = (sizeModifier: number) => {
        return Number(formData.basePrice) + Number(sizeModifier || 0);
    };
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="text-xs font-medium text-accent uppercase tracking-wider">Edit Product</span>
                                    </div>
                                    <h1 className="font-serif text-3xl md:text-4xl font-medium text-white">
                                        {pageLoading ? 'Loading...' : formData.name || 'Edit Product'}
                                    </h1>
                                    <p className="text-white/60 mt-1">Update product details and options</p>
                                </div>
                            </div>
                            <Button
                    ...
                    <div className="luxury-container py-10">
                        {pageLoading ? (
                            <div className="flex flex-col items-center justify-center py-32">
                                ...
                                <p className="mt-6 text-muted-foreground font-medium">Loading product details...</p>
                            </div>
                        ) : (
                            <form onSubmit={postProductData} className="space-y-8 max-w-5xl mx-auto">
                                    ...
                                    <div className="bg-card/80 backdrop-blur-sm border border-border/50 p-8 rounded-2xl shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] transition-shadow duration-300">
                                        ...
                                        >
                                                ...
                                                <div key={index} className="relative aspect-square group rounded-xl overflow-hidden border border-border/50 bg-secondary/30">
                                                    ...
                                                    <button
                                                        ...
                                                        className={`absolute top-2 left-2 p-2 rounded-lg backdrop-blur-sm transition-all duration-200 z-10 
                                                            ${index === 0
                                                                ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20 cursor-default'
                                                                : 'bg-white/10 text-white hover:bg-white/20 translate-y-[-10px] opacity-0 group-hover:translate-y-0 group-hover:opacity-100'
                                                            }`}
                                                    >
                                                        {index === 0 ? <Star className="w-3 h-3 fill-current" /> : <ArrowUp className="w-4 h-4" />}
                                                    </button>
                                                    ...
                                                    <button
                                                        ...
                                                        className="absolute top-2 right-2 p-1.5 bg-red-500/80 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 hover:bg-red-600 backdrop-blur-sm"
                                                    >
                                                    ...
                                                    {index === 0 && (
                                                        <div className="absolute bottom-2 left-2 px-2 py-1 bg-black/60 backdrop-blur-md rounded-md border border-white/10">
                                                            <span className="text-[10px] font-medium text-white uppercase tracking-wider">Cover</span>
                                                        </div>
                                                    )}
                                                </div>
                                            ))}
                                            ...
                                            {formData.images.length === 0 && (
                                                <div className="col-span-full flex flex-col items-center justify-center py-12 text-muted-foreground border-2 border-dashed border-border/50 rounded-xl bg-card/50">
                                                    <Upload className="w-8 h-8 mb-2 opacity-50" />
                                                    <p className="text-sm font-medium">Drag and drop images here</p>
                                                    <p className="text-xs opacity-70">or click "Add Images" button</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                ...
                                {/* Sizes */}
                                <div className={`bg-card/80 backdrop-blur-sm border border-border/50 p-8 rounded-2xl shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] transition-shadow duration-300 ${formData.categoryId === 'mattresses' || formData.categoryId === 'panels' ? 'hidden' : ''}`}>
                                    <SectionHeader icon={Ruler} title="Size Options" description="Select available sizes and set price modifiers" />

                                    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                                        {[
                                            { name: 'Small Single', dim: '75cm x 190cm', icon: BedSingle, defaultPrice: 0 },
                                            { name: 'Single', dim: '90cm x 190cm', icon: BedSingle, defaultPrice: 10 },
                                            { name: 'Small Double', dim: '120cm x 190cm', icon: BedDouble, defaultPrice: 90 },
                                            { name: 'Double', dim: '135cm x 190cm', icon: BedDouble, defaultPrice: 95 },
                                            { name: 'King', dim: '150cm x 200cm', icon: BedDouble, defaultPrice: 115 },
                                            { name: 'Queen', dim: '180cm x 200cm', icon: BedDouble, defaultPrice: 140 },
                                        ].map((sizeOption) => {
                                                            ...
                                                            <div className="bg-gradient-to-br from-primary to-primary/90 text-white px-4 py-3 rounded-lg text-center">
                                                                ...
                                                                {currentSize.priceModifier > 0 && (
                                                                    <p className="text-xs opacity-70 mt-0.5">
                                                                        £{formData.basePrice} + £{currentSize.priceModifier}
                                                                    </p>
                                                                )}
                                                            </div>
                                                        </div>
                                        ...
                                        <div className="mt-6 bg-gradient-to-br from-secondary/50 to-secondary/30 border border-border/30 rounded-xl p-5">
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex justify-between py-2 border-b border-border/30">
                                                    ...
                                                    <span className="font-semibold text-foreground">£{Number(formData.basePrice).toFixed(2)}</span>
                                                </div>
                                ...
                                <div className={`bg-card/80 backdrop-blur-sm border border-border/50 p-8 rounded-2xl shadow-[0_4px_20px_-2px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] transition-shadow duration-300 ${formData.categoryId === 'mattresses' ? 'hidden' : ''} ${formData.categoryId === 'headboards' ? 'hidden' : ''} ${formData.categoryId === 'panels' ? 'hidden' : ''}`}>
                                    ...
                                    <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                                        {[
                                            ...
                                            { name: '4 Drawers', description: 'Two drawers on the each side', icon: BedDouble, defaultPrice: 100 }
                                        ].map((storageOption) => {
